...
PRICE_SIGNAL_DEFS: list[dict[str, Any]] = [
    {
        "key": "mom_12_1",
        "label": "12-1M Return",
        "default_weight": 3,
    ...
    {
        "key": "volatility_adjusted_return_6m",
        "label": "Vol-Adj Return",
        "default_weight": 2,
        "group": "price",
        "description": "6M return ÷ annualized 6M volatility (Sharpe-like).",
    },
    {
        "key": "drawdown_from_recent_high_pct",
        "label": "Drawdown",
        "default_weight": 1,
        "group": "price",
        "description": "Current price vs 52-week high (closer to 0 = stronger).",
    },
    {
        "key": "above_200ma",
        "label": "Above 200 MA",
...
def _price_panel(series: pd.Series) -> pd.DataFrame:
    ...
    with np.errstate(divide="ignore", invalid="ignore"):
        ...
        mom_6m = np.round(
            np.where((num_6m > 0) & ~np.isnan(num_6m), (vals / num_6m - 1.0) * 100.0, np.nan), 2
        )
    ...
    with np.errstate(divide="ignore", invalid="ignore"):
        vol_adj = np.round(
            np.where(
                (ann_vol > 0) & ~np.isnan(ann_vol) & ~np.isnan(mom_6m), mom_6m / ann_vol, np.nan
            ),
            4,
        )
...
def _volume_panel(vol: pd.Series) -> pd.DataFrame:
    ...
    with np.errstate(divide="ignore", invalid="ignore"):
        v20_60 = np.round(
            np.where((long > 0) & ~np.isnan(long) & ~np.isnan(short), short / long, np.nan), 4
        )
    ...
    with np.errstate(divide="ignore", invalid="ignore"):
        vtrend = np.round(
            np.where(
                (past > 0) & ~np.isnan(past) & ~np.isnan(recent.values),
                (recent.values / past - 1.0) * 100.0,
                np.nan,
            ),
            2,
        )
...
def signals_at_cutoffs(
    ...
    *,
    ...
    `min_history_days` (an addition over the bbterminal port; None = parity
    behavior) admits a ticker at a cutoff only once BOTH its price and volume
    series start at least that many days before it — the backtest's "2 years of
    history before you're considered" rule. Young listings simply join later."""
    cutoff_idx = pd.DatetimeIndex(cutoffs)
    per_cut: dict[pd.Timestamp, list[dict[str, Any]]] = {c: [] for c in cutoffs}
    for tkr, series in price_index.items():
        ...
        for c, pos in zip(cutoffs, positions, strict=False):
            ...
            if (c - anchor).days > MAX_STALENESS_DAYS:
                continue
            if min_history_days is not None and (
                (c - series.index[0]).days < min_history_days
                or vol is None
                or len(vol) == 0
                or (c - vol.index[0]).days < min_history_days
            ):
                continue
...
def _score_category(
    df: pd.DataFrame, weights: dict[str, float], keys: list[str], col: str
) -> pd.DataFrame:
    df = df.copy()
    ...
    *,
    ...
    if exclude_incomplete and len(df):
        required = [
            ...
            if cw.get(cat, 0) != 0
            for k in ks
            if k in df.columns and weights.get(k, 0) != 0
        ]
        if required:
    ...
    *,
    ...
    if min_sector_size > 0:  # drop thin sectors before ranking
        ...
        if scored.empty:
            return pd.DataFrame()
    sect = (
        scored.groupby("sector")["momentum_score"].mean().sort_values(ascending=False).reset_index()
    )
