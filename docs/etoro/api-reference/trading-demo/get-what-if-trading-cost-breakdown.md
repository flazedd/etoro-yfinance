...
````yaml /api-reference/openapi.json post /api/v2/trading/info/demo/costs
openapi: 3.0.1
info:
  title: eToro Api
  version: v1.279.0
    ...
    post:
      ...
      description: >-
        **Rate limit:** 60 requests per 60 seconds. This is a **dedicated**
        ...
        executed now against the demo account. For `action: open`, exactly one
        of `symbol` or `instrumentId` must be provided and `leverage` must be at
        least 1. For `action: close`, `positionIds` must be non-empty.
      operationId: getCostDemo
      parameters:
        - name: x-request-id
          in: header
          required: true
          schema:
            type: string
            format: uuid
            example: 871cb2c0-c956-46a7-8078-ec9066d50235
