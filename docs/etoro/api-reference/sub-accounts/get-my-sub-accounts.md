...
Returns the authenticated user's sub-accounts. Currently the supported provider is `etoro-trading` (eToro trading sub-accounts); additional sub-account providers will be added over time.



## OpenAPI

````yaml /api-reference/openapi.json get /api/v1/sub-accounts/me/accounts
openapi: 3.0.1
info:
  title: eToro Api
  version: v1.279.0
    ...
    get:
      ...
      description: >-
        **Rate limit:** 60 requests per 60 seconds. This is the **default shared
        ...
        Returns the authenticated user's sub-accounts. Currently the supported
        provider is `etoro-trading` (eToro trading sub-accounts); additional
        sub-account providers will be added over time.
      operationId: getMySubAccounts
      parameters:
        - name: x-request-id
          in: header
          required: true
          schema:
            type: string
            format: uuid
            example: 81b6dacd-aae3-4942-bc76-b0d7f5cdc5dd
      ...
      responses:
        '200':
          description: >-
            The authenticated user's sub-accounts (e.g. an etoro-trading
            account).
          content:
            application/json:
              example:
