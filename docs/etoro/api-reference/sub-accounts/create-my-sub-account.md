...
````yaml /api-reference/openapi.json post /api/v1/sub-accounts/me/accounts
openapi: 3.0.1
info:
  title: eToro Api
  version: v1.279.0
...
tags:
  ...
  - name: Deprecated
paths:
  /api/v1/sub-accounts/me/accounts:
    post:
      tags:
        - Sub Accounts
      summary: Create My Sub-Account
      description: >-
        **Rate limit:** 60 requests per 60 seconds. This is the **default shared
        ...
        Creates a sub-account for the authenticated user with the given
        provider. Currently the supported provider is `etoro-trading` (creates
        an eToro trading sub-account); additional sub-account providers will be
        added over time. For `etoro-trading`, `additionalParams.username` is
        required.
      operationId: createMySubAccount
      parameters:
        - name: x-request-id
          in: header
          required: true
          schema:
            type: string
            format: uuid
            example: f3950b98-befb-4b84-a635-b8d08cde3ff1
      ...
      responses:
        '201':
          description: Sub-account created (etoro-trading).
          content:
            application/json:
              example:
                account:
        ...
        '202':
          description: >-
            Sub-account creation accepted and is being processed
            (etoro-trading).
          content:
            application/json:
              example:
        ...
        '409':
          description: Conflict - the sub-account already exists
        '422':
          description: >-
            Unprocessable Entity - missing required input (e.g.
            additionalParams.username for etoro-trading)
        '429':
          description: >-
