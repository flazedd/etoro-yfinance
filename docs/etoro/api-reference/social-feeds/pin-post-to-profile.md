...
Pins the specified post to the authenticated user's profile so it appears at the top of their feed. Users can pin only their own posts. The call is idempotent — pinning an already-pinned post succeeds silently.



## OpenAPI

````yaml /api-reference/openapi.json post /api/v1/posts/{postId}/pins
openapi: 3.0.1
info:
  title: eToro Api
  version: v1.279.0
    ...
    post:
      ...
      description: >-
        **Rate limit:** 60 requests per 60 seconds. This is the **default shared
        ...
        Pins the specified post to the authenticated user's profile so it
        appears at the top of their feed. Users can pin only their own posts.
        The call is idempotent — pinning an already-pinned post succeeds
        silently.
      operationId: postPostsByPostIdPins
      parameters:
        - name: x-request-id
          in: header
          required: true
          schema:
            type: string
            format: uuid
            example: e2255f3a-de4b-479e-9902-4113b645b97a
        ...
        - name: postId
          in: path
          required: true
          description: ID of the post to pin (UUID)
          schema:
            type: string
            format: uuid
      ...
      responses:
        ...
        '403':
          description: >-
            Requester is not allowed to pin the specified post (e.g., not the
            post owner)
        '404':
