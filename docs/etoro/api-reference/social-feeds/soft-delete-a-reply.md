...
Soft-deletes the authenticated user's own reply.



## OpenAPI

````yaml /api-reference/openapi.json delete /api/v1/posts/{postId}/comments/{commentId}/replies/{replyId}
openapi: 3.0.1
info:
  title: eToro Api
  version: v1.279.0
    ...
    delete:
      ...
      description: >-
        **Rate limit:** 60 requests per 60 seconds. This is the **default shared
        ...
        Soft-deletes the authenticated user's own reply.
      operationId: deletePostsByPostIdCommentsByCommentIdRepliesByReplyId
      parameters:
        - name: x-request-id
          in: header
          required: true
          schema:
            type: string
            format: uuid
            example: dc7303da-a6dd-4a95-9828-3c9e216529b2
