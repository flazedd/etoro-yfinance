...
Bookmarks a post for the authenticated user. Idempotent — saving an already-saved post succeeds silently. The save is private and visible only to the saving user. This endpoint takes no request body.



## OpenAPI

````yaml /api-reference/openapi.json post /api/v1/posts/{postId}/saves
openapi: 3.0.1
info:
  title: eToro Api
  version: v1.279.0
    ...
    post:
      ...
      description: >-
        **Rate limit:** 60 requests per 60 seconds. This is the **default shared
        ...
        Bookmarks a post for the authenticated user. Idempotent — saving an
        already-saved post succeeds silently. The save is private and visible
        only to the saving user. This endpoint takes no request body.
      operationId: postPostsByPostIdSaves
      parameters:
        - name: x-request-id
          in: header
          required: true
          schema:
            type: string
            format: uuid
            example: 2b15eca4-d180-4ab4-b071-1122a5dba716
        ...
        - name: postId
          in: path
          required: true
          description: ID of the post to save (UUID)
          schema:
            type: string
            format: uuid
