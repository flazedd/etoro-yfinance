...
Edits the authenticated user's own comment in-place. Sets editStatus to Edited. Emotion counts, repliesCount, and created are preserved. Owner is derived from the auth token.



## OpenAPI

````yaml /api-reference/openapi.json put /api/v1/posts/{postId}/comments/{commentId}
openapi: 3.0.1
info:
  title: eToro Api
  version: v1.279.0
    ...
    put:
      ...
      description: >-
        **Rate limit:** 60 requests per 60 seconds. This is the **default shared
        ...
        Edits the authenticated user's own comment in-place. Sets editStatus to
        Edited. Emotion counts, repliesCount, and created are preserved. Owner
        is derived from the auth token.
      operationId: putPostsByPostIdCommentsByCommentId
      parameters:
        - name: x-request-id
          in: header
          required: true
          schema:
            type: string
            format: uuid
            example: cf5b7ff9-f79d-4193-a8e8-5420dd59e618
...
components:
  schemas:
    CommentUpdateRequest:
      type: object
      description: >-
        Request body for updating a comment or reply. Owner is derived from the
        auth token — do not supply it.
      properties:
        message:
          type: string
