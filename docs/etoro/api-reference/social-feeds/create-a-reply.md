...
Creates a reply nested under an existing comment. Replies are 1-level deep — replying to a reply attaches to the same parent comment with an inline @mention to the reply author. Increments parent comment's repliesCount and the post's totalCommentsAndReplies.



## OpenAPI

````yaml /api-reference/openapi.json post /api/v1/posts/{postId}/comments/{commentId}/replies
openapi: 3.0.1
info:
  title: eToro Api
  version: v1.279.0
    ...
    post:
      ...
      description: >-
        **Rate limit:** 60 requests per 60 seconds. This is the **default shared
        ...
        Creates a reply nested under an existing comment. Replies are 1-level
        deep — replying to a reply attaches to the same parent comment with an
        inline @mention to the reply author. Increments parent comment's
        repliesCount and the post's totalCommentsAndReplies.
      ...
      parameters:
        - name: x-request-id
          ...
          schema:
            type: string
            format: uuid
            example: 36a66760-c0e3-4833-a6cf-b3b764289c9e
        ...
        - name: commentId
          ...
          required: true
      requestBody:
        description: Reply content. `message` is required; `attachments` is optional.
        required: true
        content:
          application/json:
            schema:
      ...
      responses:
        ...
        '422':
          description: >-
            Validation failed — missing message body or spam classification
