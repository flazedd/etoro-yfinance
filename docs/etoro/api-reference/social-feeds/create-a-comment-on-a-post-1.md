...
````yaml /api-reference/openapi.json post /api/v1/reactions/{postId}/comment
openapi: 3.0.1
info:
  title: eToro Api
  version: v1.279.0
...
tags:
  ...
  - name: Deprecated
paths:
  /api/v1/reactions/{postId}/comment:
    post:
      tags:
        - Social Feeds
      summary: Create a comment on a post
      description: >-
        **Rate limit:** 20 requests per 60 seconds. This is a **dedicated**
        ...
        [DEPRECATED — use POST /api/v1/posts/{postId}/comments instead] Creates
        a new comment on a specific discussion post. Comments can include text,
        mentions, tags, and attachments.
      operationId: postReactionsByPostIdComment
      parameters:
        - name: x-request-id
          in: header
          required: true
          schema:
            type: string
            format: uuid
            example: 85ffee65-33d7-419c-be40-cd80a1a0e4cf
        ...
        '201':
          ...
          headers:
            ...
            RateLimit-Policy:
              ...
              example: 20;w=60
        '429':
          description: Too Many Requests — the rate limit (20 requests / 60s) was exceeded.
          headers:
            RateLimit-Limit:
              description: >-
                Maximum number of requests allowed per window. This limit is
