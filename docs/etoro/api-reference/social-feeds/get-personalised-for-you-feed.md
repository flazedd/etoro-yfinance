...
Returns a personalised feed for the authenticated user ranked by followed users, followed instruments, prior engagement, and freshness. Ranking method is resolved via A/B experiment (Optimizely) and per-user overrides. Requires authentication — this feed is intrinsically personal.



## OpenAPI

````yaml /api-reference/openapi.json get /api/v1/feeds/for-you
openapi: 3.0.1
info:
  title: eToro Api
  version: v1.279.0
    ...
    get:
      ...
      description: >-
        **Rate limit:** 60 requests per 60 seconds. This is a **shared quota** —
        ...
        Returns a personalised feed for the authenticated user ranked by
        followed users, followed instruments, prior engagement, and freshness.
        Ranking method is resolved via A/B experiment (Optimizely) and per-user
        overrides. Requires authentication — this feed is intrinsically
        personal.
      operationId: getFeedsForYou
      parameters:
        - name: x-request-id
          in: header
          required: true
          schema:
            type: string
            format: uuid
            example: 2a514315-9038-43bc-a327-26d4d9255ea7
