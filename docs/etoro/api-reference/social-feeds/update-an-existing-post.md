...
Updates the authenticated user's own post in-place. The owner field is derived from the authentication token and must not be supplied in the request body. Editable fields: message, tags, mentions, attachments.



## OpenAPI

````yaml /api-reference/openapi.json put /api/v1/posts/{postId}
openapi: 3.0.1
info:
  title: eToro Api
  version: v1.279.0
    ...
    put:
      ...
      description: >-
        **Rate limit:** 60 requests per 60 seconds. This is the **default shared
        ...
        Updates the authenticated user's own post in-place. The owner field is
        derived from the authentication token and must not be supplied in the
        request body. Editable fields: message, tags, mentions, attachments.
      ...
      parameters:
        - name: x-request-id
          ...
          schema:
            type: string
            format: uuid
            example: e779a202-88bd-4e4d-a4da-d2b056ce296d
        ...
        - name: postId
          in: path
          description: ID of the post to update (UUID)
          required: true
          schema:
            type: string
            format: uuid
...
components:
  schemas:
    DiscussionUpdateRequest:
      type: object
      description: >-
        Request body for updating a post. Owner is derived from the auth token —
        do not supply it.
      properties:
        message:
          type: string
          ...
          example: >-
            Updating my view on $TSLA — still bullish but watching earnings
            closely.
        attachments:
