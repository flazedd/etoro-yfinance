...
Removes a post from the authenticated user's saved bookmarks. Idempotent — unsaving a post that was never saved succeeds silently.



## OpenAPI

````yaml /api-reference/openapi.json delete /api/v1/posts/{postId}/saves
openapi: 3.0.1
info:
  title: eToro Api
  version: v1.279.0
    ...
    delete:
      ...
      description: >-
        **Rate limit:** 60 requests per 60 seconds. This is the **default shared
        ...
        Removes a post from the authenticated user's saved bookmarks. Idempotent
        — unsaving a post that was never saved succeeds silently.
      operationId: deletePostsByPostIdSaves
      parameters:
        - name: x-request-id
          in: header
          required: true
          schema:
            type: string
            format: uuid
            example: 8b38c9a6-68e7-4a64-9ee2-54018603657e
        ...
        - name: postId
          in: path
          required: true
          description: ID of the post to unsave (UUID)
          schema:
            type: string
            format: uuid
