...
Unsubscribes the authenticated user from notifications on this post. Idempotent — unfollowing a post that was never followed succeeds silently.



## OpenAPI

````yaml /api-reference/openapi.json delete /api/v1/posts/{postId}/follows
openapi: 3.0.1
info:
  title: eToro Api
  version: v1.279.0
    ...
    delete:
      ...
      description: >-
        **Rate limit:** 60 requests per 60 seconds. This is the **default shared
        ...
        Unsubscribes the authenticated user from notifications on this post.
        Idempotent — unfollowing a post that was never followed succeeds
        silently.
      operationId: deletePostsByPostIdFollows
      parameters:
        - name: x-request-id
          in: header
          required: true
          schema:
            type: string
            format: uuid
            example: dcddcf3a-284c-4996-9508-07c5773c302f
        ...
        - name: postId
          in: path
          required: true
          description: ID of the post to unfollow (UUID)
          schema:
            type: string
            format: uuid
      ...
      responses:
        '204':
          description: Post unfollowed successfully
          headers:
            RateLimit-Limit:
              description: >-
