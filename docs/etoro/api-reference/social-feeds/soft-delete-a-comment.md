...
Soft-deletes the authenticated user's own comment. Sets isDeleted=true and blanks message text and attachments. Replies are preserved and the parent post's totalCommentsAndReplies is decremented only by the deleted node.



## OpenAPI

````yaml /api-reference/openapi.json delete /api/v1/posts/{postId}/comments/{commentId}
openapi: 3.0.1
info:
  title: eToro Api
  version: v1.279.0
    ...
    delete:
      ...
      description: >-
        **Rate limit:** 60 requests per 60 seconds. This is the **default shared
        ...
        Soft-deletes the authenticated user's own comment. Sets isDeleted=true
        and blanks message text and attachments. Replies are preserved and the
        parent post's totalCommentsAndReplies is decremented only by the deleted
        node.
      operationId: deletePostsByPostIdCommentsByCommentId
      parameters:
        - name: x-request-id
          in: header
          required: true
          schema:
            type: string
            format: uuid
            example: a45f7906-8ed1-42af-b468-fe09e6227368
