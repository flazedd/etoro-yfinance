...
Toggles a Like emotion on the specified reply for the authenticated user. emotionType defaults to Like (the only supported emotion type). The call is idempotent — liking an already-liked entity succeeds silently.



## OpenAPI

````yaml /api-reference/openapi.json post /api/v1/posts/{postId}/comments/{commentId}/replies/{replyId}/likes
openapi: 3.0.1
info:
  title: eToro Api
  version: v1.279.0
    ...
    post:
      ...
      description: >-
        **Rate limit:** 60 requests per 60 seconds. This is the **default shared
        ...
        Toggles a Like emotion on the specified reply for the authenticated
        user. emotionType defaults to Like (the only supported emotion type).
        The call is idempotent — liking an already-liked entity succeeds
        silently.
      operationId: postPostsByPostIdCommentsByCommentIdRepliesByReplyIdLikes
      parameters:
        - name: x-request-id
          in: header
          required: true
          schema:
            type: string
            format: uuid
            example: c3d85037-f58d-41eb-b861-ae244dd04eb2
        ...
        content:
          application/json:
            schema:
              type: object
              description: >-
                Empty JSON object. The body must be present but contains no
                fields.
            example: {}
      responses:
        '204':
