...
Removes the authenticated user's pin from the specified post. The call is idempotent — unpinning a post that is not currently pinned succeeds silently.



## OpenAPI

````yaml /api-reference/openapi.json delete /api/v1/posts/{postId}/pins
openapi: 3.0.1
info:
  title: eToro Api
  version: v1.279.0
    ...
    delete:
      ...
      description: >-
        **Rate limit:** 60 requests per 60 seconds. This is the **default shared
        ...
        Removes the authenticated user's pin from the specified post. The call
        is idempotent — unpinning a post that is not currently pinned succeeds
        silently.
      operationId: deletePostsByPostIdPins
      parameters:
        - name: x-request-id
          in: header
          required: true
          schema:
            type: string
            format: uuid
            example: 02109755-88d4-4d80-8052-85866cc83fdf
        ...
        - name: postId
          in: path
          required: true
          description: ID of the post to unpin (UUID)
          schema:
            type: string
            format: uuid
      ...
      responses:
        ...
        '403':
          description: Requester is not allowed to unpin the specified post
        '404':
