...
# Get Allowed Scopes (v2)

> **Rate limit:** 60 requests per 60 seconds. This is the **default shared quota** — it is shared with every other endpoint that has no dedicated limit, so requests across those endpoints all draw from the same budget.

---

Returns the permission scope names that may be assigned to agent-portfolio user tokens.



## OpenAPI

````yaml /api-reference/openapi.json get /api/v2/agent-portfolios/user-tokens/scopes
openapi: 3.0.1
info:
  title: eToro Api
  version: v1.279.0
    ...
    get:
      ...
      description: >-
        **Rate limit:** 60 requests per 60 seconds. This is the **default shared
        ...
        Returns the permission scope names that may be assigned to
        agent-portfolio user tokens.
      operationId: getAgentPortfolioAllowedScopesV2
      parameters:
        - name: x-request-id
          in: header
          required: true
          schema:
            type: string
            format: uuid
            example: b3393264-5bb1-42fc-848c-fad49ab9c25c
        ...
        '200':
          ...
          content:
            application/json:
              schema:
                $ref: >-
                  #/components/schemas/AgentPortfolioApi_GetAllowedScopesResponse
          headers:
            RateLimit-Limit:
              description: >-
      ...
      security:
        - oauth2:
            - etoro-public:agent-portfolio:read
        - oauth2:
            - etoro-public:agent-portfolio:write
components:
  schemas:
    AgentPortfolioApi_GetAllowedScopesResponse:
      type: object
      properties:
        scopes:
          type: array
          ...
          description: >-
            The permission scope names that may be assigned to agent-portfolio
            user tokens.
          example:
