...
[DEPRECATED — use PATCH /api/v2/agent-portfolios/{agentPortfolioId}/user-tokens/{userTokenId} (scope names) instead] Updates the settings of an existing user token for a agent-portfolio. At least one field must be provided.
    ...
    patch:
      ...
      description: >-
        **Rate limit:** 60 requests per 60 seconds. This is the **default shared
        ...
        (scope names) instead] Updates the settings of an existing user token
        for a agent-portfolio. At least one field must be provided.
      ...
      parameters:
        - name: x-request-id
          ...
          schema:
            type: string
            format: uuid
            example: 7e9a201e-089e-44b8-a998-66a08d2d2ecc
    ...
    UpdateUserTokenRequest:
      ...
      properties:
        scopeIds:
          ...
          deprecated: true
          description: >-
            [DEPRECATED — use scopeNames instead] An updated set of permission
            scope identifiers for the token. Available scopes: 200 =
