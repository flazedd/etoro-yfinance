...
Marks all notifications as read for the authenticated user. Rate limit: 20 requests per minute.



## OpenAPI

````yaml /api-reference/openapi.json patch /api/v1/notifications/messages
openapi: 3.0.1
info:
  title: eToro Api
  version: v1.279.0
    ...
    patch:
      ...
      description: >-
        **Rate limit:** 60 requests per 60 seconds. This is the **default shared
        ...
        Marks all notifications as read for the authenticated user. Rate limit:
        20 requests per minute.
      operationId: patchNotificationsMessages
      parameters:
        - name: x-request-id
          in: header
          required: true
          schema:
            type: string
            format: uuid
            example: 0401ed9e-cbd2-46e7-ae9d-2f490220420b
      ...
      security:
        - oauth2:
            - etoro-public:notifications:write
components:
  securitySchemes:
    oauth2:
      type: oauth2
