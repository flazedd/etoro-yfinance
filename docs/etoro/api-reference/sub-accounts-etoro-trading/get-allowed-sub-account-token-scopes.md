...
Returns the scope names that may be assigned to sub-account non-interactive user tokens (the etoro-trading-subaccount application's scopes).



## OpenAPI

````yaml /api-reference/openapi.json get /api/v1/sub-accounts/etoro-trading/user-tokens/scopes
openapi: 3.0.1
info:
  title: eToro Api
  version: v1.279.0
...
paths:
  /api/v1/sub-accounts/etoro-trading/user-tokens/scopes:
    get:
      ...
      summary: Get Allowed Sub-Account Token Scopes
      description: >-
        **Rate limit:** 60 requests per 60 seconds. This is the **default shared
        ...
        Returns the scope names that may be assigned to sub-account
        non-interactive user tokens (the etoro-trading-subaccount application's
        scopes).
      operationId: getSubAccountUserTokenScopes
      parameters:
        - name: x-request-id
          in: header
          required: true
          schema:
            type: string
            format: uuid
            example: 6ca32556-dc30-41dd-a113-0cc3122aa873
      ...
      responses:
        '200':
          description: Allowed scopes retrieved successfully
          content:
            application/json:
              schema:
                $ref: >-
                  #/components/schemas/EtoroTradingSubAccountsOperationsApi_GetAllowedScopesResponse
          headers:
            RateLimit-Limit:
              description: >-
...
components:
  schemas:
    EtoroTradingSubAccountsOperationsApi_GetAllowedScopesResponse:
      type: object
      properties:
        scopes:
          type: array
          ...
          description: The scope names that may be assigned to sub-account user tokens.
          example:
