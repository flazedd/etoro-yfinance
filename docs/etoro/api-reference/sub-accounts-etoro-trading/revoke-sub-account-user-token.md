...
Revokes a sub-account user token. Revocation is idempotent, so revoking a token that does not exist also succeeds.



## OpenAPI

````yaml /api-reference/openapi.json delete /api/v1/sub-accounts/etoro-trading/user-tokens/{userTokenId}
openapi: 3.0.1
info:
  title: eToro Api
  version: v1.279.0
...
paths:
  /api/v1/sub-accounts/etoro-trading/user-tokens/{userTokenId}:
    delete:
      ...
      summary: Revoke Sub-Account User Token
      description: >-
        **Rate limit:** 60 requests per 60 seconds. This is the **default shared
        ...
        Revokes a sub-account user token. Revocation is idempotent, so revoking
        a token that does not exist also succeeds.
      operationId: deleteSubAccountUserToken
      parameters:
        - name: x-request-id
          in: header
          required: true
          schema:
            type: string
            format: uuid
            example: 524a5e25-7745-4262-b38d-4f62133c86c5
        ...
        - name: userTokenId
          ...
          schema:
            type: string
            format: uuid
          description: The unique identifier of the user token to revoke.
      ...
      responses:
        '204':
          description: >-
            User token revoked successfully (also returned when the token did
            not exist)
          headers:
            RateLimit-Limit:
              description: >-
        ...
        '400':
          description: >-
            Bad Request — missing, malformed, or undecryptable x-sub-account-id
            header
          content:
            application/json:
              schema:
