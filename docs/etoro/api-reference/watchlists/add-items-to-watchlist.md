...
Adds new items to an existing watchlist.



## OpenAPI

````yaml /api-reference/openapi.json post /api/v1/watchlists/{watchlistId}/items
openapi: 3.0.1
info:
  title: eToro Api
  version: v1.279.0
    ...
    post:
      ...
      description: >-
        **Rate limit:** 20 requests per 60 seconds. This is a **shared quota** —
        ...
        Adds new items to an existing watchlist.
      operationId: postWatchlistsByWatchlistIdItems
      parameters:
        - name: x-request-id
          in: header
          required: true
          schema:
            type: string
            format: uuid
            example: e66f1c38-69bd-4c65-99a4-2c2868f59c9e
        ...
        - name: watchlistId
          ...
          example: '12345'
      requestBody:
        description: Items to add to the watchlist
        required: true
        content:
          application/json:
            schema:
